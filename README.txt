...
3. Built-in web console. Default address: http://127.0.0.1:62930
...
6. Supports proxy management: add, edit through config, and delete proxy entries.
7. Supports editing the frpc.toml configuration file.
8. Supports viewing and clearing frpc, service, update, and web logs.
9. Supports account settings. By default, no username or password is set and first access skips login. After an account is set, the login session is valid for 24 hours and can be logged out manually.
10. Supports Root manager status display: web mode, frpc status, and web address.
...
3. 内置 web 后台，默认地址：http://127.0.0.1:62930
4. 支持 web 后台启动、停止 frpc。
5. 支持最小内存运行模式：启动 frpc 后关闭 web 后台，只保留 frpc 和轻量守护。
6. 支持代理管理，可添加、通过配置编辑、删除代理配置。
7. 支持编辑 frpc.toml 配置文件。
8. 支持查看、清除 frpc、service、update、web 等运行日志。
9. 支持账号设置：默认无用户名和密码，首次进入后台免登录；设置账号密码后登录有效期为 24 小时，可主动退出登录。
10. 支持模块管理器状态显示：web 模式、frpc 运行状态、web 地址。
...
- frpc: downloaded frpc binary after installation or update; not included in the module package
  frpc：安装或更新后下载得到的 frpc 二进制文件，模块包内不自带
- frpc.toml: frpc configuration file
  frpc.toml：frpc 配置文件
- frpc.log: frpc runtime log
  frpc.log：frpc 运行日志
- webui.sh: web console service script
  webui.sh：web 后台服务脚本
- frpc.cgi: web console page script
  frpc.cgi：web 后台页面脚本
- service.sh: boot startup and background guard script
  service.sh：开机启动和后台守护脚本
- action.sh: Root manager action button script
  action.sh：模块管理器操作按钮脚本

- webroot/index.html: Root manager web button page, opens the local frpc web console.
  webroot/index.html：Root 管理器 web 按钮页面，用于打开本机 frpc web 控制台。
